...
- **`contexts/`** — `AuthContext` (session/profile), `GenerationContext` (story-generation state machine, SSE → ruby parse → save), `KanjiContext` (known-state + per-kanji exposure counts; `refreshKanjiExposures` is called by `StoryReadButton` so the header total score updates live), `DictionaryContext` (lookup cache).
