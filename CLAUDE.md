...
- **`contexts/`** — `AuthContext` (session/profile), `GenerationContext` (story-generation state machine, SSE → ruby parse → save), `KanjiContext` (known-state + per-kanji exposure counts; `refreshKnownKanji` also refreshes the exposure map so toggling a kanji known/unknown updates the header total live, and `refreshKanjiExposures` is called by `StoryReadButton` so marking a story read does the same), `DictionaryContext` (lookup cache).
...
- **`hooks/useAudioPlayer`** — playback + sentence-level highlight sync against `audio.sentences[]` (each entry carries the sentence's character offset in `content` and its time offset in ms).
...
- `functions/generate-audio/` — Azure Neural TTS. Input is `{story_id, title, title_annotations, content, content_annotations}`; the title is spoken plainly (no bookmarks, since it's rendered as a separate `<h2>` that isn't highlighted), then content is bookmarked sentence-by-sentence (`s0…sN`) and paragraph-by-paragraph (`p0…pN`) with `start` recorded as a *character offset into content*. Persists path + sentence/paragraph sync points into `stories.audio` JSONB and uploads MP3 to the `story-audio` Storage bucket. Returns 500 *"Azure TTS is not configured"* if `AZURE_SPEECH_KEY` or `AZURE_SPEECH_REGION` is unset. The SSML scanner mirrors `client/src/lib/storySegments.ts` so the audio bookmarks align 1:1 with the rendered sentences.
...
- `stories` — per-user stories. Columns include `title`, `content`, `paragraphs`, `topic`, `formality` (`impolite`/`casual`/`polite`/`keigo`), `content_type` (`story`/`dialogue`/`essay`), `filters` JSONB, `allowed_kanji` (TEXT — concatenated kanji string, not JSON), `difficulty` JSONB, `audio` JSONB nullable (`{path, duration_ms, voice, version, paragraphs, sentences}` where `paragraphs[i].start`/`sentences[i].start` are character offsets into `content`; older `version: 3` rows additionally had a `tokens` field which the player ignores — version mismatches trigger regeneration on next play), `explanations` JSONB nullable (`{ "start-end": { "<chip_id>": { version, messages } } }` where `messages[0]` is the hidden chip prompt seed), `read_count` (INT NOT NULL, defaults 0; weights kanji-exposure aggregation so re-reads count), `first_read_at` / `last_read_at`, `created_at`. RLS-scoped.
